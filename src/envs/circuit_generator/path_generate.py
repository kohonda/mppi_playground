...
def make_csv_paths(csv_file: str, DL=0.1, offset=True) -> np.ndarray:
    ...
    for i in range(len(df)):
        ...
        if norm != 0:
            ...
            direction_vector = np.array(
                [1, 0]
            )  # if norm is zero, default direction vector
    ...
    def calculate_angles(path: np.ndarray) -> np.ndarray:
        ...
        initial_direction_vector = np.array(
        ...
        initial_angle = np.arctan2(
            initial_direction_vector[1], initial_direction_vector[0]
        )
...
def make_side_lane(
    road: np.ndarray, lane_width: float
